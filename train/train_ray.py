...
os.environ['MLFLOW_EXPERIMENT_NAME'] = 'cnn-summarizer-fine-tune-ray'
...
def preprocess_data(examples):
    ...
    return pd.DataFrame.from_dict(model_inputs)
...
batch_encoder = BatchMapper(preprocess_data, batch_format="pandas", batch_size='default')

cnn_ds_cleaned = cnn_ds.filter(lambda example: (len(example['article']) >= 500) and (len(example['highlights']) >= 20))
cnn_ds_train = cnn_ds_cleaned['train'].shuffle(seed = 42).select(range(TRAIN_SIZE))
cnn_ds_valid = cnn_ds_cleaned['validation'].shuffle(seed = 42).select(range(EVAL_SIZE))
cnn_ds_train = ray.data.from_huggingface(cnn_ds_train)
cnn_ds_valid = ray.data.from_huggingface(cnn_ds_valid)
